package cn.alchemy3d.geom
{
	...
	public class Mesh3D extends EventDispatcher
	{
		...
		private function setFaces( fs:Vector.<Triangle3D> ):Vector.<uint>
		{
			...
			for each( f in fs )
			{
				Library.memory.writeUnsignedInt( f.material.pointer );
			}
			...
			for each( f in fs )
			{
				Library.memory.writeUnsignedInt( f.texture ? f.texture.pointer : 0 );
			}
		...
		protected function callAlchemy():Array
		{
			...
			return Library.alchemy3DLib.initializeMesh( vp, fp, uvp, mp, tp, rmp,  numVertices, numFaces );
		}
