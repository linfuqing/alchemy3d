...
DWORD A3DS_Chunk( UCHAR ** buffer, WORD wID, DWORD dwLength )
{
	...
	tmp = ( * buffer );
		...
		AS3_Trace(AS3_String("begin"));
		AS3_Trace(AS3_Int(( * buffer )));
		AS3_Trace(AS3_String("end"));
		AS3_Trace(AS3_Int(end));
	...
	while( ( * buffer ) <= end )
	{
		...
		AS3_Trace(AS3_String("once"));
		AS3_Trace(AS3_Int(( * buffer )));
		AS3_Trace(AS3_Int(wID));
		AS3_Trace(AS3_Int(wIDRead));
		AS3_Trace(AS3_String("length"));
		AS3_Trace(AS3_Int(dwBlockLength));
		AS3_Trace(AS3_Int(dwLength));
...
void A3DS_LoadData( UCHAR ** buffer_ptr, DWORD length )
{
	WORD	wID = 0;
	...
	if ( ! ( dwBlockLength = A3DS_Chunk( buffer_ptr, CHUNK_OBJMESH, dwLength ) ) )
	{
		//可能的文件I/O错误
		exit( TRUE );
	}
