...
VOID CChessRules::GetMoves(BASECHESSBOARD& ChessBoard, const INT x, const INT y, const BOOL bAttack)
{
	INT i, j;
	CHESSMANTYPE From = ChessBoard.gChess[y][x];
	...
	case BLACK_CAR:
		for(i = x + 1, j = y; i < CHESS_WIDTH   ; i++)
		{
			...
			{
				if( !( bAttack && IsSameSide(From,ChessBoard.gChess[j][i]) ) )
					AddMove(x, y, i, j);
				break;
			}
		}
	...
	case RED_PAWN:
		i = x;
		j = y - 1;
		if( j > 0               && !IsSameSide(From, ChessBoard.gChess[j][i]) )
			AddMove(x, y, i, j);
		...
		if(y < 5)
		{
			...
			i = x - 1;
			if( i > 0           && !IsSameSide(From, ChessBoard.gChess[j][i]) )
				AddMove(x, y, i, j);
			i = x + 1;
			if( i < CHESS_WIDTH && !IsSameSide(From, ChessBoard.gChess[j][i]) )
				AddMove(x, y, i, j);
		}
		break;
	case BLACK_PAWN:
		i = x;
		j = y + 1;
		if( j < CHESS_HEIGHT    && !IsSameSide(From, ChessBoard.gChess[j][i]) )
			AddMove(x, y, i, j);
...
BOOL CChessRules::TestMove(BASECHESSBOARD &ChessBoard, const INT nFromX, const INT nFromY, const INT nToX, const INT nToY, const BOOL bAttack)
{
