...
BOOL CChessDlg::OnInitDialog()
{
	...
	m_ChessBoard.SetChessBoard(INIT_CHESSBOARD);

	CMoveGenerator* pMoveGenerator = new CMoveGenerator;
	CEveluation*    pEveluation    = new CEveluation;

	m_pSearchEngine = new CNegamaxEngine;
	m_pSearchEngine->SetSearchDepth(3);
	m_pSearchEngine->SetMoveGenerator(pMoveGenerator);
	m_pSearchEngine->SetEveluator(pEveluation);
...
void CChessDlg::OnPaint()
{
	...
	for(INT i = 0; i < CHESS_WIDTH; i++)
	{
		...
		{
			if(m_ChessBoard.gChess[j][i] == NO_CHESS)
				continue;

			pt.x = i * GRILLEWIDTH  + OFFSETWIDTH;
			pt.y = j * GRILLEHEIGHT + OFFSETHEIGHT;

			m_Chessman.Draw(&MemDC, CHESS_TYPE_SUBSCRIPT[m_ChessBoard.gChess[j][i]], pt, ILD_TRANSPARENT);
		}
	}
...
void CChessDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	m_BackupChessBoard.SetChessBoard(&m_ChessBoard);

	if( point.x > 0 && point.y > 0 && point.x < m_nBoardWidth && point.y < m_nBoardHeight && IsSide(PLAYER_SIDE, m_ChessBoard.gChess[y][x]) )
	{
		m_MoveChess.From.x = x;
		m_MoveChess.From.y = y;
		m_MoveChess.Type   = m_ChessBoard.gChess[y][x];
		
		m_ChessBoard.gChess[y][x] = NO_CHESS;
...
void CChessDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		bTurnSide = TRUE;
		m_ChessBoard.gChess[y][x] = m_MoveChess.Type;
	}
	else
		m_ChessBoard.SetChessBoard(&m_BackupChessBoard);
	...
	if(bTurnSide)
	{
		...
		m_pSearchEngine->SearchAGoodMove(m_ChessBoard);
		CString sNodeCount;
...
void CChessDlg::OnNewGame()
{
	...
	if(NewGameDlg.DoModal() == IDOK)
	{
		...
		{
		case 1:
			m_pSearchEngine = new CAlphaBetaEngine;
			break;
		default:
			m_pSearchEngine = new CNegamaxEngine;
			break;
		...
		m_pSearchEngine->SetMoveGenerator(new CMoveGenerator);
		m_pSearchEngine->SetEveluator(new CEveluation);
	}
	else
		return;
