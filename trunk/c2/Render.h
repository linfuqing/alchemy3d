...
static INLINE uint32 computePixelColor( float r, float g, float b, float a, float u, float v, Texture * texture )
{
	float * bitmapData = texture->bitmapData;
	int tColor = texture->bitmapDataP[(int)(u * (texture->width - 1) + 0.5) + ( (int)(v * (texture->height - 1) + 0.5) * texture->width )];
...
{
	RenderVertex	* tmpV;
	uint32			* gfxBuffer = view->gfxBuffer, color;
			...
			for ( yi = cyStart; yi <= cyEnd; yi ++ )
			{
				...
				if ( cxEnd >= 0 && cxStart <= nw && ( ( zStart > 0 && zStart < 1 )  || ( zEnd > 0 && zEnd < 1 ) ) )
				{
					if (cxStart == cxEnd)
					{
						...
						if( zStart > 0 && zStart < 1 && zStart < zBuffer[pos] )
						{
							gfxBuffer[pos] = computePixelColor( r, g, b, a, uStart, vStart, texture );
							zBuffer[pos] = zStart;
						}
					}
					else
					{
						...
						for ( xi = cxStart; xi < cxEnd; xi ++ )
						{
							...
							if( currZ > 0 && currZ < 1 && currZ < zBuffer[pos] )
							{
								gfxBuffer[pos] = computePixelColor( r, g, b, a, currU, currV, texture );
								zBuffer[pos] = currZ;
							}
						...
						if( zEnd > 0 && zEnd < 1 && zEnd < zBuffer[pos] )
						{
							gfxBuffer[pos] = computePixelColor( r, g, b, a, uEnd, vEnd, texture );
							zBuffer[pos] = zEnd;
						}
					}
				}
			...
			for ( yi = cyStart; yi <= cyEnd; yi ++ )
			{
				...
				if ( cxEnd >= 0 && cxStart <= nw && ( ( zStart > 0 && zStart < 1 )  || ( zEnd > 0 && zEnd < 1 ) ) )
				{
					if (cxStart == cxEnd)
					{
						...
						if( zStart > 0 && zStart < 1 && zStart < zBuffer[pos] )
						{
							gfxBuffer[pos] = computePixelColor( rStart, gStart, bStart, aStart, uStart, vStart, texture );
							//gfxBuffer[pos] = ((int)aStart << 24) + ((int)rStart << 16) + ((int)gStart << 8) + (int)bStart;
							zBuffer[pos] = zStart;
						}
					}
					else
					{
						...
						for ( xi = cxStart; xi < cxEnd; xi ++ )
						{
							...
							if( currZ > 0 && currZ < 1 && currZ < zBuffer[pos] )
							{
								gfxBuffer[pos] = computePixelColor( currR, currG, currB, currA, currU, currV, texture );
								//gfxBuffer[pos] = ((int)currA << 24) + ((int)currR << 16) + ((int)currG << 8) + (int)currB;
								zBuffer[pos] = currZ;
							}
						...
						if( zEnd > 0 && zEnd < 1 && zEnd < zBuffer[pos] )
						{
							gfxBuffer[pos] = computePixelColor( rEnd, gEnd, bEnd, aEnd, uEnd, vEnd, texture );
							//gfxBuffer[pos] = ((int)aEnd << 24) + ((int)rEnd << 16) + ((int)gEnd << 8) + (int)bEnd;
							zBuffer[pos] = zEnd;
						}
					}
