...
INT CEveluation::Eveluate(BASECHESSBOARD& ChessBoard, CHESSSIDE Side)
{
	...
	for(i = 0; i < CHESS_WIDTH; i++)
	{
		for(j = 0; j < CHESS_HEIGHT; j++)
		{
			...
			{
				ChessType = ChessBoard.gChess[j][i];
				//GetRelatePiece(ChessBoard, i, j);
				m_nPositionCount = 0;
				CChessRules::GetMoves(ChessBoard, i, j, FALSE);
				for(k = 0; k < m_nPositionCount; k++)
				{
					TargetType = ChessBoard.gChess[gRelatePosition[k].y][gRelatePosition[k].x];
					if(TargetType == NO_CHESS)
						m_Flexibility.gChess[j][i]++;
					else
					{
						if( ChessType >> SIDE_SHIFT == TargetType >> SIDE_SHIFT )
							m_Guard.gChess[gRelatePosition[k].y][gRelatePosition[k].x]++;
						else
						{
							m_Attract.gChess[gRelatePosition[k].y][gRelatePosition[k].x]++;
							m_Flexibility.gChess[j][i]                                  ++;
	...
	for(i = 0; i < CHESS_WIDTH; i++)
	{
		for(j = 0; j < CHESS_HEIGHT; j++)
		{
			if(ChessBoard.gChess[j][i] != NO_CHESS)
			{
				...
				///PartI
				m_ChessValue.gChess[j][i] ++;
				m_ChessValue.gChess[j][i] += m_FlexValue[nSubscript] * m_Flexibility.gChess[j][i];

				if(ChessType == RED_PAWN)
					m_ChessValue.gChess[j][i] += RED_PAWN_ADDITIONAL_VALUE[j][i];
				if(ChessType == BLACK_PAWN)
					m_ChessValue.gChess[j][i] += BLACK_PAWN_ADDITIONAL_VALUE[j][i];

				///PartII
				nHalfValue = m_BaseValue[nSubscript];

				m_ChessValue.gChess[j][i] += nHalfValue;
				...
				if(m_Attract.gChess[j][i])
				{
					...
					{
						///Fuck This:
						m_ChessValue.gChess[j][i] += ChessType == King ? -20 : -(nHalfValue * 2);
						if(m_Guard.gChess[j][i])
							m_ChessValue.gChess[j][i] += nHalfValue;
					}
					else
					{
						...
						///Fuck This:
						m_ChessValue.gChess[j][i] -= m_Guard.gChess[j][i] ? nHalfValue : nHalfValue * 10;
					}

					m_ChessValue.gChess[j][i] -= m_Attract.gChess[j][i];
				}
				else if(m_Guard.gChess[j][i])
					//And Fuck This:
					m_ChessValue.gChess[j][i] += 5;

				///PartIII:
				if(ChessSide == RED_SIDE)
					nRedValue   += m_ChessValue.gChess[j][i];
				else
					nBlackValue += m_ChessValue.gChess[j][i];
			}
		}
	}
...
INT CEveluation::IsGameOver(BASECHESSBOARD &ChessBoard, const INT nDepth, const CHESSSIDE Side, const BOOL bIsCurrentSide)
{
	INT i, j;
